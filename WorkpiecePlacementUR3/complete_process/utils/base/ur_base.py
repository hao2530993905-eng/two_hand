...
class UR_BASE:
    ...
    def move_j_ik(
        ...
        try:
            if self.control_backend == "rtde":
                if not self.rtde_c.getInverseKinematicsHasSolution(target):
                    return MotionResult(False, "no inverse-kinematics solution for TCP pose", time.monotonic() - start)
                command_ok = self.rtde_c.moveJ_IK(target, speed_value, acceleration_value, True)
            else:
                program_error = self._urscript_motion_error()
                if program_error:
                    return MotionResult(False, program_error, time.monotonic() - start)
