#!/usr/bin/env python3
"""Detect raised rectangular parts using aligned RealSense depth images.

Remove the parts and press B to collect a multi-frame depth background. The
detector then keeps pixels that are closer to the camera than that background
by a configurable height. RGB is used only for visualization.
...
# OpenCV HSV hue ranges (H is 0..179). Red wraps around the hue boundary.
COLOR_HUE_RANGES = {
    "red": ((0, 10), (170, 179)),
    "orange": ((11, 22),),
    "yellow": ((23, 35),),
...
def hsv_color_mask(bgr_image, color_name, min_saturation, min_value):
    """Return a binary mask for a named color preset."""
    hsv = cv2.cvtColor(bgr_image, cv2.COLOR_BGR2HSV)
    result = np.zeros(hsv.shape[:2], dtype=np.uint8)
    for hue_min, hue_max in COLOR_HUE_RANGES[color_name]:
        partial = cv2.inRange(
            hsv,
            (hue_min, min_saturation, min_value),
            (hue_max, 255, 255),
        )
        result = cv2.bitwise_or(result, partial)
    return result
...
class DepthBackgroundRectangleDetector:
    def __init__(self):
        ...
        self.max_height_mm = int(rospy.get_param("~max_height_mm", 150))
        ...
        self.close_size = int(rospy.get_param("~close_size", 17))
        self.min_area = float(rospy.get_param("~min_area", 3000))
        self.target_color = str(
            rospy.get_param("~target_color", "none")
        ).strip().lower()
        self.color_min_saturation = int(
            rospy.get_param("~color_min_saturation", 80)
        )
        self.color_min_value = int(rospy.get_param("~color_min_value", 50))
        valid_colors = ("none",) + tuple(COLOR_HUE_RANGES)
        if self.target_color not in valid_colors:
            raise ValueError(
                "target_color must be one of {}".format(", ".join(valid_colors))
            )
        if self.min_area < 0.0:
            raise ValueError("min_area must be non-negative")
        if not 0 <= self.color_min_saturation <= 255:
            raise ValueError("color_min_saturation must be in [0, 255]")
        if not 0 <= self.color_min_value <= 255:
            raise ValueError("color_min_value must be in [0, 255]")
        ...
        self.dataset_name = str(
            rospy.get_param("~dataset_name", "obb_dataset")
        ).strip()
        if (
        ...
        # Receive the two large image streams independently, as the main
        # workflow's SensorCollector does.  message_filters did not deliver
        # synchronized callbacks with the current RealSense /camera streams
        # even though both ROS connections were established.
    ...
    def update_latest_pair_locked(self):
        if self.latest_color is None or self.latest_depth is None:
            return
        delta = abs(
            self.message_stamp(self.latest_color)
            - self.message_stamp(self.latest_depth)
        )
        self.latest_pair_delta = delta
        if delta <= 0.10:
            self.latest_pair = (self.latest_color, self.latest_depth)

    def color_callback(self, color_message):
        with self.pair_lock:
            self.latest_color = color_message
            self.update_latest_pair_locked()
    ...
    def read_controls(self):
        ...
        self.max_height_mm = max(
            self.min_height_mm,
            cv2.getTrackbarPos("Max height mm", CONTROL_WINDOW),
        )
    ...
    def initialize_dataset(self):
        ...
        if not yaml_path.exists():
            yaml_path.write_text(
                "path: {}\ntrain: images/train\nval: images/val\n\n"
                "names:\n  0: workpiece\n".format(self.dataset_dir.resolve()),
                encoding="utf-8",
            )
    ...
    def process_latest(self):
        with self.pair_lock:
            ...
            pair_delta = self.latest_pair_delta
        if pair is None:
            rospy.logwarn_throttle(
                3.0,
                "Waiting for synchronized color/depth: color=%s depth=%s "
                "latest_delta=%s s",
                has_color,
                has_depth,
                "n/a" if pair_delta is None else "{:.6f}".format(pair_delta),
            )
            if self.show_gui:
                cv2.waitKey(1)
            return
        ...
        try:
            ...
            cv2.putText(
                debug,
                "ROI {} {} {} {}  height={}..{} mm  color={}  min_area={:.0f}".format(
                    x1, y1, x2 - x1, y2 - y1,
                    self.min_height_mm, self.max_height_mm,
                    self.target_color, self.min_area),
